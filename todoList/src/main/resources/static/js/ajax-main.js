...
-> 비동기 작업이 완료되었을 때 실행할 콜백함수를 지정하고,
  ...
  - Pending(대기 중) : 비동기 작업이 완료 되지 않은 상태
...
function getTotalCount() { // 함수 정의
  ...
  .then( response => { // 서버에서 응답을 받으면,
                      // 이 응답(reponse)을 텍스트 형식으로 변환하는 콜백합수
                      // 매개변수 response : 비동기 요청에 대한 응답이 담긴 객체
    console.log(response);
    // response.text() : 응답 데이터를 문자열/쉇자 형태로 변환한 결과를 가지는
    //                    Promise 객체 반환
...
const selectTodoList = () => {
  ...
  .then(todoList => {
    ...
    // 만약 rsp.text() 사용했다면 문자열 (JSON이 그대로 노출)
...
changeComplete.addEventListener("click", () => {
  ...
  const obj = {"todoNo": todoNo, "comlpete" : complete};
  // {"todoNo" : 2, :"complete" : 'Y'}
