...
SELECT CHATTING_ROOM_NO
	...
	,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2 
		...
		R.PARTICIPANT, -- NULL이 아닌 경우 (로그인한 사람 오픈맴버일때는 참가지 타겟번호)
		R.OPEN_MEMBER-- NULL 인 경우 (내가 오픈 맴버가 아닐때는 OPEN_MEMBER가 타켓번호)
		) TARGET_NO	
	,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
		WHERE R2.CHATTING_ROOM_NO = R.CHATTING_ROOM_NO
		AND R2.OPEN_MEMBER = 1),
...
-- 게시글
SELECT BT.BOARD_NAME, B.BOARD_NO, COUNT(*) AS READ_COUNT, 
