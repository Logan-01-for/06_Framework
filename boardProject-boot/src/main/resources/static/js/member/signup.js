...
sendAuthKeyBtn.addEventListener("click", () => {
  ...
  // 새로운 인증번호 발급을 원하는것이기 때문애
  // 새로 발급받은 인증번호 확인 전까진 checkObj.authKey는 false
  checkObj.authKey = false;
  // 인증번호 발급 관련된 메시지 지우기ㄴ
  authKeyMessage.innerText = "";
...
checkAuthKeyBtn.addEventListener("click", () => {
  ...
  if( min === 0 && sec === 0) { // 타이머가 00:00인 경우
    alert("인증번호 입력 제한 시간을 초과하였습니다. 다시 발금해주세요.");
    return;
  }
...
signUpForm.addEventListener("submit", e => {
  ...
  for(let key in checkObj) { // checkObj 요소의 key 값을 순서대로 꺼내옴
    ...
    if(!checkObj[key]){ // 현재 접근중인 checkObj[key] 의 value 값이 fales인 경우
