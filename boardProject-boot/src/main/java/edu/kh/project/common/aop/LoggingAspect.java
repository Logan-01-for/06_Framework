...
public class LoggingAspect {
	...
	public Object aroundServiceImpl(ProceedingJoinPoint pjp) throws Throwable{
		...
		// Throwable 주요 자식으로 Exception (예외) - 개발자 처리할 수 있는 문제
		...
		String methodName = pjp.getSignature().getName(); // ex) mainPage(), login ..메서드명
		...
		Object obj = pjp.proceed(); // 전/ 후로 나누는 기준점
	...
	// @AfterThrowing : 메서드가 예외를 던진 후에 실행되느 Advice 를 정의
	@AfterThrowing(pointcut = "@annotation(org.sprinframework.transacion.annotation.Transactional)", 
			throwing = "ex")
