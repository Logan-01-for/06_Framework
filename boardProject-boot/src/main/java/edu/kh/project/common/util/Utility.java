...
public class Utility {
	...
	public static String fileRename(String originalFileName) {
		...
		// originalFileName == 짱구.jpg
			...
			String newFileName = date + "_" + number + ext;
			
			return newFileName;
		
	}
	
	// 랜덤 비밀 번호 발급 메서드 (관리자용)
	public static String generatePassword() {
		...
		for(int i=0; i < CODE_LENGTH; i++) {
			int index = random.nextInt(CHARACTERS.length()); // CHARACTERS 길이 (62)
			// random.nextInt(62) 는 0부터 61 사이의 난수를 생성
			
			randomCode.append(CHARACTERS.charAt(index));
			// CHARACTERS 문자열의 index 위치에 있는 문자를 반환
			// ex) index가 0이면 'A' , index 가 61이면 '9'를 반환
