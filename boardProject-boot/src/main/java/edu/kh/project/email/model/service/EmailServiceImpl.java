...
public class EmailServiceImpl implements EmailService{
	...
	// JavaMailSender : 실제 메링 발송을 담당하는 객체 (EmailConfig 설정이 적용된 객체)
	private final SpringTemplateEngine templateEngine;
	// SpringTemplateEngine : 타임리프를 이용해서 html코드 -> java 코드 변환
	...
	@Transactional(rollbackFor=Exception.class) // 메서드레벨에서도 이용 가능(해당
	private boolean storeAuthkey(Map<String, String> map) {
		...
		// 2. UPDATE 수행 시 0이 반환됨 == UPDATE안됨 == INSERT된 내용이 없었다
		//		INSERT 수행하면 된다
		if(result == 0) {
	...
	// 인증 번호 발급 메서드
	// UUID 를 사용하여 인증키 생성
	// (Universally Unique IDentifier) : 전 세계 에서 고유한 식별자를 생성하기 위한 표준
	// 매우 낮은 확률로 중복되는 식별자를 생성
	// 주로 데이터베이스 기본키, 고유한 식별자를 생성 할 때 사용
