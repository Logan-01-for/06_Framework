...
public class ParameterController {
	...
	@GetMapping("main") // 	/param/main GET 방식 요청 메핑 
	 ...
	 * required : 입력된 name 속성 값 파라미터 필수 여부 지정 (기본값 true)
	 * -> required=true 인 파라미터가 존재하지 않는다면 400(Bad Request) 에러 발생
	 * -> "" (빈무자열) 일 때는 에러 발생 X 
	 * (파라미터가 존재하는 것이 아니라 name 속성값="" 로 넘어오기 때문에)
	 * 
	...
	public String paramTest2(@RequestParam("title") String title,
							...
							// 형변환이 필요한 경우에는 파라미터  value  값 필수 작성되있어야함!
	...
	public String paramTest4(/*@ModelAttribute*/ MemberDTO inputMember) {
		...
		return "redirect:main"; // 상대 경로 작성법 = /param/test/result
		// == "redirect:/param/main"
		// (현재위치가 중요! 
		//-> 현재 경로의 가장 마지막 부분을 redirect: 이 주소로 변셩하기 때문에)
	}
}	
